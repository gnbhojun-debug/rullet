...
STATE_IDLE = "IDLE"
STATE_SPINNING_PRIZE = "SPINNING_PRIZE"
STATE_PRIZE_REVEALED = "PRIZE_REVEALED"
...
class TextInput:
    ...
    def handle_event(self, event: pygame.event.Event) -> bool:
        ...
        allowed = "0123456789,-~ "
        if event.unicode and event.unicode in allowed and len(self.text) < 80:
    ...
    def draw(self, surface: pygame.Surface) -> None:
        ...
        text = self.text if self.text else "1-50"
...
class GachaLotteryApp:
    ...
    def __init__(self) -> None:
        ...
        self.rank_spin_start = 0
        self.rank_spin_duration = 1400
        self.rank_roll_next = 0
        self.rank_switch_start = 0
        self.rank_switch_interval = 45
        self.rank_previous_value: int | None = None
        self.rank_spin_value: int | None = None
        self.rank_bounce_start = -9999
        self.auto_tens_at = 0
        self.auto_ones_at = 0

        self.message = "Start를 누르면 경품 등수와 당첨 번호가 자동 추첨됩니다."
    ...
    def rank_accent(self, rank: int | None = None, now: int | None = None) -> tuple[int, int, int]:
        if rank is None:
            return LOGO_GRAY
        if rank == 1:
            if now is None:
                now = pygame.time.get_ticks()
            return RAINBOW_COLORS[(now // 110) % len(RAINBOW_COLORS)]
        if rank == 2:
            return (255, 216, 64)
        if rank == 3:
            return (216, 224, 232)
        if rank == 4:
            return LOGO_TEAL_LIGHT
        if rank == 5:
            return LOGO_ORANGE_LIGHT
        return LOGO_GRAY
    ...
    def apply_number_pool(self) -> None:
        if self.state in {
            STATE_SPINNING_PRIZE,
            STATE_PRIZE_REVEALED,
            STATE_SPINNING_TENS,
            STATE_TENS_REVEALED,
            STATE_SPINNING_ONES,
        }:
        ...
        self.rank_spin_value = None
        self.rank_previous_value = None
        self.rank_switch_start = 0
        self.auto_tens_at = 0
        self.auto_ones_at = 0
        self.show_unwon_list = False
    ...
    def handle_start(self) -> None:
        ...
        if self.state == STATE_IDLE:
            ...
            self.selected_prize = random.randrange(len(PRIZE_NAMES))
            self.rank_spin_start = now
            selected_rank = self.selected_prize + 1
            self.rank_spin_duration = 1160 if selected_rank in {4, 5} else 1400
            self.rank_roll_next = now
            self.rank_switch_start = now
            self.rank_switch_interval = 45
            self.rank_previous_value = random.randint(1, len(PRIZE_NAMES))
            self.rank_spin_value = random.randint(1, len(PRIZE_NAMES))
            self.rank_bounce_start = -9999
            self.auto_tens_at = 0
            self.auto_ones_at = 0
            self.tens_digit.reset()
            self.ones_digit.reset()
            self.state = STATE_SPINNING_PRIZE
            self.device_pulse_until = now + 2400
            self.set_message("경품 등수 추첨 중...", 1800)
            self.spawn_start_particles()
            return

    def digit_spin_durations(self) -> tuple[int, int]:
        rank = self.selected_prize + 1 if self.selected_prize is not None else 0
        if rank in {4, 5}:
            return (1150, 1250)
        return (2300, 2450)
    ...
    def start_tens_spin(self, now: int) -> None:
        if self.target_number is None:
            return
        tens_duration, _ = self.digit_spin_durations()
    ...
    def start_ones_spin(self, now: int) -> None:
        if self.target_number is None:
            return
        _, ones_duration = self.digit_spin_durations()
    ...
    def spawn_rainbow_fireworks(self) -> None:
        """1등 경품 전용 무지개 폭죽입니다."""
        burst_centers = [
            (300 + ROULETTE_X_OFFSET, 190),
            (455 + ROULETTE_X_OFFSET, 145),
            (620 + ROULETTE_X_OFFSET, 165),
            (760 + ROULETTE_X_OFFSET, 245),
            (360 + ROULETTE_X_OFFSET, 385),
            (690 + ROULETTE_X_OFFSET, 390),
        ]
    ...
    def spawn_prize_rank_effect(self, rank: int) -> None:
        """오각형 등수 슬롯에서 등수가 확정될 때 나오는 효과입니다."""
        center = (526 + ROULETTE_X_OFFSET, 156 + ROULETTE_Y_OFFSET)
        if rank == 1:
            ...
            colors = [(255, 216, 64), (255, 238, 150), LOGO_ORANGE_LIGHT]
            count = 88
            speed_range = (130, 330)
            size_choices = [3, 4]
        elif rank == 3:
            colors = [(216, 224, 232), (245, 248, 250), LOGO_GRAY]
            count = 76
            speed_range = (115, 290)
            size_choices = [2, 3, 4]
        else:
            colors = [LOGO_ORANGE_LIGHT, LOGO_TEAL_LIGHT, LOGO_GRAY_LIGHT]
            count = 48
            speed_range = (90, 230)
            size_choices = [2, 3]
        ...
        for _ in range(count):
            ...
            self.particles.append(
                Particle(
                    center[0] + random.uniform(-22, 22),
                    center[1] + random.uniform(-18, 18),
                    math.cos(angle) * speed,
                    math.sin(angle) * speed,
                    random.uniform(0.75, 1.45),
                    1.45,
                    random.choice(size_choices),
                    random.choice(colors),
                    random.choice(["star", "circle", "heart"]),
                    gravity=random.uniform(80, 190),
                )
            )
    ...
    def update(self, dt: float, now: int) -> None:
        ...
        if self.state == STATE_SPINNING_PRIZE:
            progress = clamp((now - self.rank_spin_start) / self.rank_spin_duration, 0.0, 1.0)
            if now >= self.rank_roll_next:
                self.rank_previous_value = self.rank_spin_value
                self.rank_spin_value = random.randint(1, len(PRIZE_NAMES))
                self.rank_switch_start = now
                self.rank_switch_interval = int(34 + ease_out_cubic(progress) * 132)
                self.rank_roll_next = now + self.rank_switch_interval
            if progress >= 1.0 and self.selected_prize is not None:
                rank = self.selected_prize + 1
                self.rank_previous_value = self.rank_spin_value
                self.rank_switch_start = now
                self.rank_spin_value = rank
                self.rank_bounce_start = now
                self.shake_until = now + 360
                self.state = STATE_PRIZE_REVEALED
                self.auto_tens_at = now + 720
                self.device_pulse_until = now + 1700
                self.set_message(f"{rank}등 경품이 선택되었습니다.", 1800)
                self.spawn_prize_rank_effect(rank)

        if self.state == STATE_PRIZE_REVEALED and self.auto_tens_at and now >= self.auto_tens_at:
            self.auto_tens_at = 0
        ...
        if tens_finished and self.state == STATE_SPINNING_TENS:
            ...
            self.auto_ones_at = now + (
                250 if (self.selected_prize is not None and self.selected_prize + 1 in {4, 5}) else 520
            )
    ...
    def draw_selected_prize_banner(
        ...
        rank_revealed = self.state not in {STATE_IDLE, STATE_SPINNING_PRIZE}
        rank = self.selected_prize + 1 if self.selected_prize is not None and rank_revealed else None
        accent = self.rank_accent(rank, now) if rank_revealed else LOGO_GRAY
        glow = pygame.Surface((banner.width + 26, banner.height + 26), pygame.SRCALPHA)
        pygame.draw.rect(
            glow,
            (*accent, (68 + int(75 * pulse)) if rank_revealed else 38),
            glow.get_rect(),
            border_radius=13,
        )
        ...
        if self.selected_prize is None or not rank_revealed:
            banner_text = "Start를 누르면 등수가 먼저 공개됩니다"
        else:
    ...
    def draw_gacha_device(self, surface: pygame.Surface, now: int) -> None:
        ...
        crystal = [
            (526 + ROULETTE_X_OFFSET, 106 + ROULETTE_Y_OFFSET),
            (594 + ROULETTE_X_OFFSET, 164 + ROULETTE_Y_OFFSET),
            (556 + ROULETTE_X_OFFSET, 214 + ROULETTE_Y_OFFSET),
            (496 + ROULETTE_X_OFFSET, 214 + ROULETTE_Y_OFFSET),
            (458 + ROULETTE_X_OFFSET, 164 + ROULETTE_Y_OFFSET),
        ]
        rank_revealed = self.state not in {STATE_IDLE, STATE_SPINNING_PRIZE}
        rank = self.selected_prize + 1 if self.selected_prize is not None and rank_revealed else None
        accent = self.rank_accent(rank, now)
        pygame.draw.polygon(surface, (0, 0, 0), [(x, y + 8) for x, y in crystal])
        pygame.draw.polygon(surface, PANEL_LIGHT, crystal)
        pygame.draw.polygon(surface, accent, crystal, 7)
        pygame.draw.polygon(surface, LOGO_GRAY_LIGHT, crystal, 2)

        slot_rect = pygame.Rect(
            478 + ROULETTE_X_OFFSET,
            116 + ROULETTE_Y_OFFSET,
            98,
            96,
        )
        previous_clip = surface.get_clip()
        surface.set_clip(slot_rect)

        def draw_rank_text(value: int | None, center: tuple[int, int], scale: float = 1.0) -> None:
            text = "?" if value is None else f"{value}등"
            image = self.fonts["result"].render(text, True, accent)
            if abs(scale - 1.0) > 0.01:
                image = pygame.transform.scale(
                    image,
                    (
        ...
        center_x = 526 + ROULETTE_X_OFFSET
        center_y = 164 + ROULETTE_Y_OFFSET
        if self.state == STATE_SPINNING_PRIZE:
            local = clamp((now - self.rank_switch_start) / max(1, self.rank_switch_interval), 0.0, 1.0)
            fall = ease_out_cubic(local) * slot_rect.height
            draw_rank_text(self.rank_previous_value, (center_x, int(center_y + fall)))
            draw_rank_text(self.rank_spin_value, (center_x, int(center_y - slot_rect.height + fall)))
        else:
            rank_t = clamp((now - self.rank_bounce_start) / 520, 0.0, 1.0)
            rank_scale = 1.0
            if 0.0 < rank_t < 1.0:
                rank_scale = 1.0 + math.sin(rank_t * math.pi) * (1 - rank_t) * 0.32
            draw_rank_text(self.rank_spin_value, (center_x, center_y), rank_scale)
    ...
    def draw_status(self, surface: pygame.Surface) -> None:
        ...
        if self.message_until and pygame.time.get_ticks() > self.message_until:
            if self.state == STATE_IDLE:
                text = "Start를 누르면 등수와 번호가 자동 추첨됩니다."
            elif self.state == STATE_PRIZE_REVEALED:
                text = "등수 공개 완료. 번호 룰렛 준비 중..."
    ...
    def draw_congratulations(self, surface: pygame.Surface, now: int) -> None:
        if self.state != STATE_FINAL_REVEALED or self.target_number is None:
            return
        ...
        number_panel = pygame.Rect(right_center[0] - 125, right_center[1] - 92, 250, 150)
        panel_accent = self.rank_accent(self.last_winner_rank, now)
        pygame.draw.rect(surface, (0, 0, 0), number_panel.move(8, 10), 3, border_radius=12)
        pygame.draw.rect(surface, panel_accent, number_panel, 4, border_radius=12)
        draw_text(
            surface,
            "당첨 번호",
            self.fonts["subtitle"],
